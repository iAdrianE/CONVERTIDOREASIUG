...
export const processResearchLetter = async (filePath, outputDir) => {
  try {
  ...
  const cleanAndOptimizeHtml = (htmlContent) => {
    ...
    $("strong, em").each((_, elem) => {
      const $elem = $(elem);
      if ($elem.is("strong")) {
        $elem.replaceWith($("<bold>").html($elem.html()));  // Convierte <strong> a <b>
      } else if ($elem.is("em")) {
        $elem.replaceWith($("<italic>").html($elem.html()));  // Convierte <em> a <i>
      }
    });
...
// Buscar el primer párrafo después del boxedtext inicial
let firstTextSection = null;
$("p.boxedtext").each((index, elem) => {
  // Busca todos los párrafos después de <boxedtext>, hasta encontrar un h3, h4 o h5
  const nextParagraphs = $(elem).nextUntil("h3, h4, h5", "p");  // Solo los párrafos hasta un h3, h4 o h5
  
  if (nextParagraphs.length > 0 && !processedSections.has("First Text Section")) {
    // Almacena todos los párrafos encontrados como párrafos separados
    firstTextSection = nextParagraphs.map((_, p) => `<p>${$(p).html()}</p>`).get().join("");  // Agrega cada párrafo como <p> y combínalos sin saltos de línea

    processedSections.add("First Text Section");
    sections.push({ title: "First Text Section", content: firstTextSection });
    
    // Mostrar los resultados de depuración
    console.log("Contenido del boxed text:", $(elem).html());
    console.log("Contenido de los párrafos siguientes:", firstTextSection);
  }
});
...
// Incluir primer texto inicial si existe
const foundSection = sections.find((sec) => sec.title === "First Text Section");
if (foundSection) {
  const initialSec = body.ele("sec");
  initialSec.txt(decode(foundSection.content.trim())); // Inserta el texto directamente sin <p>
}
...
images.forEach((image, index) => {
  ...
  caption.ele("label").txt(`Figure ${index + 1}.`);  // Deja la etiqueta vacía
